...
export const UserWallet: React.FC = () => {
  const [balance, setBalance] = useState(500); // Default fallback
  ...
  const loadWalletData = async () => {
    ...
    try {
      ...
      try {
        ...
        setBalance(walletData.wallet_balance || 500);
      } catch (walletError) {
        console.warn('Failed to load wallet balance:', walletError);
        setBalance(500); // Keep default
      }
    ...
    },
    {
      ...
      render: (value: number, row: Transaction) => (
        <span className={`font-semibold ${
          row.type === 'bid' ? 'text-red-600' : 'text-green-600'
        }`}>
